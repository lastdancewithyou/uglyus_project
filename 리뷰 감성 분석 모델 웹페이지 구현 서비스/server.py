...
class Report:
    ...
    def get_report_by_day(self):
        ...
        score_message = f'평균 감정 점수는 {target_score}점이며, {interval}일 전 대비 {change_score if (change_score) > 0 else change_score * (-1)}점이 ' + ('상승하였습니다' if change_score > 0 else '하락하였습니다.')
    ...
    def topic_score(self, last, current):
        ...
        for i in range(5):
            ...
            changed = (pos_current - neg_current * 13.5) - (pos_last - neg_last * 13.5)
            topic_scores.append(pos_current - neg_current * 13.5)
    ...
    def get_plot(self, min, max, topic_scores, change, is_week = True):
        ...
        title_font = {'size': 30, 'family': 'sans-serif', 'weight':'bold'}
        subplot_font = {'size': 30, 'family': 'sans-serif'}

        plt.subplot(3, 1, 1)
        sns.lineplot(x = trend['x_axis'], y = trend['modified_score'], marker = 'o', color = 'black')
        plt.xlabel(x_label, subplot_font)
        plt.ylabel('Sentiment score', subplot_font)
        plt.ylim([-5, 100])
        plt.title(title_name, title_font)
        plt.annotate('Lowest score\n' + ': ' + str(round(min_score,2)), xy = (min_axis, min_score), xytext = (min_axis, min_score - 0.15),
                    arrowprops = dict(facecolor = 'red', arrowstyle = '->, head_length = 0.5, head_width = 0.5'), color = 'red',
                    weight='bold', fontsize=20)
        plt.annotate('Highest score\n' + ': ' + str(round(max_score,2)), xy = (max_axis, max_score), xytext = (max_axis, max_score + 0.1),
                    arrowprops = dict(facecolor = 'red', arrowstyle = '->, head_length = 0.5, head_width = 0.5'), color = 'red',
                    weight='bold', fontsize=20)
        ...
        plt.subplot(3, 1, 2)
        sns.barplot(x = x, y = topic_scores, color = 'pink')
        ...
        plt.annotate('Lowest score\n' + ': ' + str(round(min_topicscore,3)), xy = (min_index, min_topicscore), 
                     xytext = (min_index - 0.3, min_topicscore), color = 'green', weight='bold', fontsize=20)
        plt.annotate('Highest score\n' + ': ' + str(round(max_topicscore,3)), xy = (max_index, max_topicscore),
                      xytext = (max_index - 0.3, max_topicscore), color = 'green', weight='bold', fontsize=20)
                
        plt.subplot(3, 1, 3)
        sns.barplot(x = x, y = change, color = 'blue')
        ...
        plt.annotate('Lowest changed score\n' + ': ' + str(round(min_changescore,3)), xy = (min_changeindex, min_changescore), 
                     xytext = (min_changeindex - 0.3, min_changescore), color = 'green', weight='bold', fontsize=20)
        plt.annotate('Highest changed score\n' + ': ' + str(round(max_changescore,3)), xy = (max_changeindex, max_changescore), 
                     xytext = (max_changeindex - 0.3, max_changescore), color = 'green', weight='bold', fontsize=20)
